...
// // const getPlatformFiles = (platform: LocalPlatform): Promise<string[]> => {
// //   return fs.readdir(`./roms/${platform.name}`).catch(err => {
// //     if (err.code !== 'ENOENT') throw err;
// //     return [];
// //   });
// // };

// const filterPlatformFiles = (platform: LocalPlatform) => (
//   files: string[]
// ): Promise<MatchFile[]> => {
...
//       })
//       .map(file => {
//         const parts = file.match(/(^.+)\.(.+)$/);
//         return {
//           full: `./roms/${platform.name}/${file}`,
//           name: parts[1],
//           ext: parts[2],
//           match: parts[1].toLowerCase()
//         };
//       })
//   );
// };
...
// ) => (files: MatchFile[]): Promise<PlatformMatch> => {
//   if (!files.length) {
//     return Promise.resolve({
//       id: platform.id,
//       matched: null
...
//     matched:
//       (matched && {
//         file: files[matched.bestMatchIndex],
